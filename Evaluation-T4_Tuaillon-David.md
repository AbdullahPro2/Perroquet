...
- Répartir les captures d'écran dans l'ensemble des parties. Par exemple, la capture d'écran sur le compte rendu de la semaine pourrait être dans "Simulation : Chaque post correspond à une semaine qui passe" ou encores les captures d'écran de "flop, burn-out" dans la partie scénario.

-Indiquer que dans la partie "fonctionnalités et scénarios avancés", les éléments ne sont pas implémentés. En effet, le temps verbal et la formulation utilisée suppose que ces fonctionnalités sont déjà implémentée dans le prototype.


# Propositions d'amélioration du jeu

- Ajouter un petit bouton dans l'interface qui ouvre une page de règles et d'explications pour les débutant qui seraient perdus.

- Ajouter la possibilité de basculer vers un thème clair.
...
Les fichiers sont organisé dans une arborescence cohérente et le projet est divisé en dossiers src,
layouts, pages, assets... Ce qui traduit d'une bonne structure. Le code respecte les conventions de camelCase et beaucoup de commentaires sont présents.

Pour ce qui est des commits, ils sont réguliers et chaque commit détaille le message de la modification. Toutefois les noms des commits sont composés d'anglais et de français mélangé avec les différents membres du projet.
...
j'ai appris quel types de contenu il faut poster sur les différentes platformes (qui utilisent des algorithmes différents) et quelles sont les conséquences sur l'audience de changer brutalement de bord politique.

Autres justifications :

L'installation est très simple, il suffit de clicker sur le lien de la page pour y accéder. Toutefois si on veut l'installer nous même en local, cela est possible et les instructions sont claires et détaillées dans le fichier installation.md.

En arrivant sur le jeu, il n'y a pas de règles à proprement parler mais le joueur comprend globalement les enjeus et les thèmes abordés. De plus je n'ai pas constaté de bug sur l'interface durant ma période d'essai.
...
La phrase de présentation est longue mais très bien formulée et arrive a résumer parfaitement le but et les enjeux du logiciel.

Les captures d'écran sont nombreuses et présentes mais, comme dit précédemment, auraient pu être réparties dans chaque partie au lieu qu'une seule.

Enfin pour la partie installation, il n'y a qu'une seule phrase avec le lien du site, ce qui est très bien mais il n'y a pas de lien vers le fichier installation.md pour les utilisateurs qui souhaiteraient installer le jeu en local.
