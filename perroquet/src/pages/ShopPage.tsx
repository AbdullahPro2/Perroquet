        ...
        <div className="border-2 border-slate-600 rounded-xl p-4 md:p-6 relative bg-slate-900 flex flex-col">
          {hasTrendAnalyzer && (
            <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center rounded-xl z-10">
              <span className="bg-emerald-900/50 text-emerald-400 font-bold px-4 py-2 rounded-lg border border-emerald-500/50 text-sm md:text-base">
                ✔️ Déjà acquis
              </span>
            </div>
          )}
          ...
          <p className="text-xs md:text-sm text-slate-400 mt-1 md:mt-2 mb-4 md:mb-6 flex-1">
            Découvrez en temps réel les thèmes qui buzzent sur la plateforme.
          </p>
          <div className="flex justify-between items-center border-t border-slate-700 pt-4">
            <span className="font-black text-amber-500 text-lg md:text-base">
              200 €
            </span>
            <button
        ...
        <div className="border-2 border-slate-600 rounded-xl p-4 md:p-6 relative bg-slate-900 flex flex-col">
          {hasPoliticalDoc ? (
            ...
            <div className="flex flex-col h-full">
              <h3 className="font-bold text-sm md:text-lg text-rose-500 flex items-center gap-2 mb-2 md:mb-3 leading-tight">
                📂 Dossier Confidentiel : Méta X-Sphere
              </h3>
              <div className="bg-slate-800 p-3 rounded-lg border border-slate-700 text-[10px] md:text-xs leading-relaxed text-slate-300 flex-1">
                <p className="mb-2">
                  <span className="font-bold text-white">Conclusion :</span>{" "}
                  L'algorithme de X-Sphere récompense le clivage émotionnel
                  fort.
                </p>
                <p className="mb-3">
                  <span className="text-emerald-400 font-bold">La Meta :</span>{" "}
                  S'aligner sur des positions d'
                  <span className="font-bold text-white">
                    Extrême (Droite ou Gauche)
                  </span>{" "}
                  avec un ton{" "}
                  <span className="font-bold text-white">Radical</span> garantit
                  des vues massives.
                </p>
                <div className="bg-rose-950/50 border border-rose-500/30 p-2 rounded">
                  <p className="text-rose-400 font-bold text-[10px] md:text-xs">
                    ⚠️ AVERTISSEMENT PSYCHOLOGIQUE
                  </p>
                  <p className="text-rose-300 mt-1">
                    Jouer un rôle extrême qui contredit vos convictions réelles
                    endommagera gravement votre Santé Mentale. La rentabilité a
                    un coût moral.
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col h-full">
              <h3 className="font-bold text-base md:text-lg leading-tight">
                Bords Politiques Optimaux sur X-Sphere
              </h3>
              <p className="text-xs md:text-sm text-slate-400 mt-1 md:mt-2 mb-4 md:mb-6 flex-1">
                Achetez les résultats de cet audit indépendant. Il révèle
                secrètement quels bords politiques l'algorithme met en avant.{" "}
                <br />
                <span className="text-indigo-400 text-[10px] md:text-xs font-bold mt-2 block">
                  (Débloque de nouvelles options de création)
                </span>
              </p>
              <div className="flex justify-between items-center border-t border-slate-700 pt-4">
                <span className="font-black text-amber-500 text-lg md:text-base">
